...
test("v2 settings migrate additively to v4 without losing valid values or bindings", () => {
  const storage = new MemoryStorage({
    "hollow-crown-settings": JSON.stringify({
      version: 2,
      audio: { music: 0.41 },
  ...
  const settings = new SettingsStore(storage);
  assert.equal(settings.get("version"), 4);
  assert.equal(settings.get("audio.music"), 0.41);
  assert.equal(settings.get("gameplay.difficulty"), "story");
  assert.equal(settings.get("gameplay.lastDifficultyId"), "story");
...
test("v4 gamepad rebinding round-trips without resurrecting the default button", () => {
  const storage = new MemoryStorage();
  const first = new SettingsStore(storage);
